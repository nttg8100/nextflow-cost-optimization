...
test-fastqc-singularity: ${HOME}/.pixi/bin/pixi
	${HOME}/.pixi/bin/pixi run nextflow run benchmark_computing_r-e standard esource.nf -profile singularity \
...
test-fastqc-docker-amd64: ${HOME}/.pixi/bin/pixi
	${HOME}/.pixi/bin/pixi run nextflow run benchmark_comput-e standard ing_resource.nf -profile docker \
		--run_fastqc --outdir results/test_fastqc \
		-resume -with-report report_test_fastqc.html
...
test-fastqc-docker-arm64: ${HOME}/.pixi/bin/pixi
	${HOME}/.pixi/bin/pixi run nextflow run benchmark_computing_resource.n-e standard f -profile docker,emulate_amd64 \
		--run_fastqc --outdir results/test_fastqc \
		-resume -with-report report_test_fastqc.html
...
vep/114_GRCh38: ${HOME}/.pixi/bin/pixi
	mkdir -p vep
	touch vep/vep_cache
	cd vep && ${HOME}/.pixi/bin/pixi run -e standard aws s3 --no-sign-request cp s3://annotation-cache/vep_cache/114_GRCh38 114_GRCh38 --recursive

vep/vep_cache.tar: vep/114_GRCh38
	tar -cvf vep/vep_cache.tar -C vep/114_GRCh38 .

upload-vep-cache: vep/vep_cache.tar
	${HOME}/.pixi/bin/pixi run -e standard aws s3 cp vep/114_GRCh38 s3://io-benchmark/ --endpoint-url http://localhost:9000 --recursive
	${HOME}/.pixi/bin/pixi run -e standard aws s3 cp vep/vep_cache.tar s3://io-benchmark/ --endpoint-url http://localhost:9000
	
mount-s3-vep-cache: ${HOME}/.pixi/bin/pixi
	mkdir -p ./mnt/vep_cache
	${HOME}/.pixi/bin/pixi run -e mount mount-s3 --endpoint-url http://localhost:9000 --region us-east-1 --force-path-style io-benchmark ./mnt/vep_cache --read-only

inputs/HG00620.applyRecalibration.SNP.applyFilter.vcf.gz: ${HOME}/.pixi/bin/pixi
	unset $(shell env | grep '^AWS_' | cut -d= -f1); \
	mkdir -p inputs; \
	cd inputs && ${HOME}/.pixi/bin/pixi run -e standard aws s3 --no-sign-request cp \
	s3://1000genomes/technical/working/20140609_bgi_chs_validation/sample_level_calls/HG00620.applyRecalibration.SNP.applyFilter.vcf.gz .
	cd inputs && ${HOME}/.pixi/bin/pixi run -e standard aws s3 --no-sign-request cp \
	s3://1000genomes/technical/working/20140609_bgi_chs_validation/sample_level_calls/HG00620.applyRecalibration.SNP.applyFilter.vcf.gz.tbi .

test-vep-local: ${HOME}/.pixi/bin/pixi
	${HOME}/.pixi/bin/pixi run -e standard nextflow run benchmark_vep.nf -profile docker --vep_cache "./vep/114_GRCh38"

test-vep-mount-cache: ${HOME}/.pixi/bin/pixi
	${HOME}/.pixi/bin/pixi run -e standard nextflow run benchmark_vep.nf \
